...
void gaussian_filter(uchar* image, uchar* out, int X, int Y, double sigma)
{
	...
	for (y = 0; y<Y; y++)
	{
		...
		for (i = 0; i<n; i++)
		{
			...
			// if ((x == X-1) && (y == Y-1)){
			// 	printf("x: %d y: %d i: %d kernel: %f val: %f\n", x, y, i, kernel[i], val);
			// 	// printf("x: %d y: %d z: %d val: %f\n", x, y, i, val);
			// }
...
double chain(int from, int to, double * Ex, double * Ey,double * Gx, double * Gy, int X, int Y)
{
	...
	// if (from == 298237 && to == 298239)printf("from: %d to: %dGy: %f Gx: %f \nExf: %f Eyf: %f Ext %f Eyt %f\n", from, to, Gy[from], Gx[from], Ex[from], Ey[from], Ex[to], Ey[to]);
...
void compute_gradient(double * Gx, double * Gy, double * modG,uchar * image, int X, int Y)
{
	...
	for (y = 1; y<(Y - 1); y++)
	{
		...
		// if ((x == X-2) && (y == Y-2)){
		// 	printf("Gx: %f Gy: %f modG: %f\n", Gx[x + y*X], Gy[x + y*X], modG[x + y*X]);
		// }
	}
}
...
void chain_edge_points(int * next, int * prev, double * Ex,	double * Ey,double * Gx, double * Gy, int X, int Y)
{
	...
	for (x = 2; x<(X - 2); x++){   /* 2 pixel margin to include the tested neighbors */
		for (y = 2; y<(Y - 2); y++){
			...
			{
				// printf("i: %d Ex: %f Ey: %f\n", x + y*X, Ex[x + y*X], Ey[x + y*X]);
				...
				for (j = -2; j <= 2; j++)
				{
					...
					// if (x+y*X == 300797){printf("i: %d Ex: %f Ey: %f to: %d from: %d\n", x + y*X, Ex[x + y*X], Ey[x + y*X], to, from);}
					// if (x+y*X == 300797){printf("from: %d to: %d\n", from, to);}
					double s = chain(from, to, Ex, Ey, Gx, Gy, X, Y);  /* score from-to */
					// if (x+y*X == 298237){printf("s: %f from: %d to: %d\n", s, from, to);}
			...
			// printf("i: %d next: %d prev: %d\n",x + y*X, next[x + y*X], prev[x + y*X]);
		}
	}
	// if ((x==638) && (y==477)){printf("Chain id: %d next: %d prev: %d\n", x+y*X, next[x+y*X], prev[x+y*X]);}
}
	...
	double * modG,	int X, int Y,double th_h, double th_l)
{
	...
	// printf("If i: %d next: %d prev: %d modG %f valid: %d\n", i, next[i], prev[i], modG[i], valid[i]);
	// if (i == 1121) {
	// 		printf("Thresh id: %d next: %d prev: %d\n", i, next[i], prev[i]);
	// 	}
